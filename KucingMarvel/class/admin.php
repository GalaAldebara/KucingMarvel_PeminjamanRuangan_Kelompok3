...
class admin extends user
{
    public function tambah()
    {
        ...
        $query = "INSERT INTO user (nim, nama, jurusan, no_telp, level, password) VALUES ('$nim', '$nama', '$jurusan', '$no_telp', '$level', '$hashedPassword')";
    ...
    public function historiPeminjaman() //admin
    {
        ...
        if (!empty($searchTerm)) {
            $query = "SELECT p.nim,u.nama, p.status, r.nama_ruang, DAY( p.tanggal) as Hari, DATE_FORMAT( p.tanggal, '%d-%m-%Y') as tanggal, p.id_ruang, p.kegiatan, p.kode_jp_mulai, p.kode_jp_selesai,jp.jp_mulai,jp.jp_selesai FROM peminjaman p
            INNER JOIN ruang r ON r.id_ruang = p.id_ruang
            INNER JOIN user u ON u.nim = p.nim
            INNER JOIN jp ON jp.kode_jp = p.kode_jp_mulai
            WHERE (p.status = 'diterima' OR p.status = 'ditolak') AND tanggal LIKE '%$searchTerm'";
        } else {
            $query = "SELECT p.nim, u.nama, p.status, r.nama_ruang, DAY( p.tanggal) as Hari, DATE_FORMAT( p.tanggal, '%d-%m-%Y') as tanggal, p.id_ruang, p.kegiatan, p.kode_jp_mulai, p.kode_jp_selesai,jp.jp_mulai,jp.jp_selesai FROM peminjaman p
            INNER JOIN ruang r ON r.id_ruang = p.id_ruang
            INNER JOIN user u ON u.nim = p.nim
    ...
    public function requestPeminjaman()
    {
        ...
        if (!empty($searchTerm)) {
            $query = "SELECT p.id_peminjaman, p.nim, u.nama, p.status, r.nama_ruang, DAY( p.tanggal) as Hari, DATE_FORMAT( p.tanggal, '%d-%m-%Y') as tanggal, p.id_ruang, p.kegiatan, p.kode_jp_mulai, p.kode_jp_selesai 
                                    FROM peminjaman p
                                    INNER JOIN ruang r ON r.id_ruang = p.id_ruang
                                    INNER JOIN user u ON u.nim = p.nim 
                                    INNER JOIN jp ON jp.kode_jp = p.kode_jp_mulai
                                    WHERE p.status = 'belum' AND tanggal LIKE '%$searchTerm'
        ...
        while ($peminjaman = mysqli_fetch_array($result)) {
            ...
            // Isi tabel
            echo "<tr>";
            echo "<td style='display:none;'>{$peminjaman['id_peminjaman']}</td>";
            echo "<td>{$peminjaman['nim']}</td>";
            echo "<td>{$peminjaman['nama']}</td>";
            echo "<td>{$peminjaman['nama_ruang']}</td>";
            // echo "<td>$namedDay</td>";
