                ...
                foreach ($mergedData as $ruang) {
                    foreach ($ruang as $hariData) {
                        ...
                        for ($jam_count = 1; $jam_count <= 11; $jam_count++) {
                            ...
                            // Modify this block to include a link for "Available" cells
                            if ($availability == "Available") {
                                // Build the link URL with room, day, and time parameters
                                $linkURL = "index.php?page=peminjaman&room=" . urlencode($hariData['kode_ruang']) . "&day=" . urlencode($_POST['day']) . "&time=" . urlencode($jam_count) . "&date=" . urlencode($_POST['date']);

                                // Output the link
                                echo "<td class='bg-success' style='color:white;'><a href='$linkURL'>$availability</a></td>";
                            } else if ($availability == "Unavailable") {
                                echo "<td class='bg-warning' style='color:white;'>$availability</td>";
                            } else {
                                echo "<td calss='bg-danger' style='color:white;'>$availability</td>";
                            }
                        }
